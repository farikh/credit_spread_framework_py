...
def load_bars_from_db(timeframe, start=None, end=None, limit=4000):
    ...
    1. When only start date is specified:
       ...
       - Plus additional records before the start date to reach a total of 'limit' bars
    
    2. When only end date is specified:
       - All records on or before the end date, up to 'limit' bars total
    ...
    # Case 2: Only start date is specified or both dates are specified
    else:
        ...
        # If we already have enough bars, return them
        if len(range_df) >= limit:
            return range_df.tail(limit)
        
        # Otherwise, get additional bars before the start date
        remaining_bars = limit - len(range_df)
        if remaining_bars > 0 and start is not None:
            history_query = f"""
                SELECT TOP {remaining_bars}
                    bar_id,
                    timestamp,
                    [open]   AS open_price,
                    [high]   AS high,
        ...
        # If we don't need additional bars or start is None, return the range_df
        return range_df
    
    # Execute the query for Case 1
    with engine.begin() as conn:
